...
const get_billableAndUnbillable_ByIdFromDB = async (billable_unbillagble_Id) => {
    const [result] = await dbConnection.promise().query(`SELECT * FROM billableAndUnbillable WHERE id = ?`,
        [billable_unbillagble_Id]
    );
    return result;
};
...
const validate_billableAndUnbillableble_Data = (data) => {
...
export const billableAndUnbillable_create = async (req, res) => {
    try {
        ...
        const validationError = validate_billableAndUnbillableble_Data(req.body);

        if (validationError) {
            return res.status(400).json({ message: validationError });
        };

        const [billableAndUnbillableRow] = await dbConnection.promise().query(
            `INSERT INTO billableAndUnbillable (
        ...
        res.status(200).json({
            message: "Successfully added billableAndUnbillable details",
            data: { id: billableAndUnbillableRow.insertId, ...req.body }
        })
    } catch (error) {
        res.status(500).json({ message: 'Failed to create billableAndUnbillable details', error: error.message });
    }
};
